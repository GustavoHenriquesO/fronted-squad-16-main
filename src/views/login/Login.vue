...
// Função handleLogin real
const handleLogin = async () => {
  isLoading.value = true;
  errorMessage.value = ''; // Limpa erros antigos

  try {
    // A chamada real do Axios
      ...
      <div class="w-full max-w-md">
        ...
        <VForm @submit.prevent="handleLogin" v-model="isValid">
          ...
          <span class="flex flex-col w-full align-center mt-2 gap-2">
            <p class="text-sm">
              Esqueceu sua senha?
              <router-link to="/" class="text-blue">Clique aqui</router-link>
            </p>
            <p class="text-sm">
              Não tem uma conta?
              <router-link to="/register" class="text-blue">Crie uma</router-link>
            </p>
          </span>
        </VForm>
      </div>
